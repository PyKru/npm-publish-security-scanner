...
const allowlist = readJsonIfExists(path.join(process.cwd(), 'rules', 'allowlist.json'), { allowedFiles: [] });
...
function isAllowlisted(filePath) {
  return (allowlist.allowedFiles || []).some((entry) => entry === filePath || filePath.endsWith(entry));
}

function pushFinding(pathName, rule, severity) {
  if (isAllowlisted(pathName)) return;
  allFindings.push({ path: pathName, rule, severity });
  if (severity === 'HIGH') criticalViolations += 1;
}
...
function checkPackedFiles(files) {
  ...
  files.forEach(({ path: filePath, size }) => {
    ...
    if (BLOCKED_EXACT.has(basename)) {
      pushFinding(filePath, `Blocked file: ${basename}`, 'HIGH');
      return;
    }

    if (BLOCKED_EXTENSIONS.has(ext)) {
      pushFinding(filePath, `Blocked extension: ${ext}`, 'HIGH');
      return;
    }

    BLOCKED_PATTERNS.forEach(({ rx, name, severity }) => {
      if (rx.test(filePath)) pushFinding(filePath, name, severity);
    });

    if (size && size > MAX_FILE_MB * 1024 * 1024) {
      pushFinding(filePath, `File too large: ${(size / 1024 / 1024).toFixed(2)} MB`, 'MEDIUM');
    }
  });

  const totalMB = totalBytes / 1024 / 1024;
  if (totalMB > MAX_TOTAL_MB) {
    pushFinding('(package)', `Package too large: ${totalMB.toFixed(2)} MB`, 'MEDIUM');
  }
...
function validatePublishConfiguration() {
  ...
  if (!fs.existsSync('.npmignore') && !pkg.files) {
    issues.push('No .npmignore AND no "files" field — all project files may publish.');
    criticalViolations += 1;
  }
  if (Array.isArray(pkg.files) && pkg.files.includes('*')) {
...
console.log(c('cyan', '\n▶ Validating publish configuration...'));
try {
  validatePublishConfiguration().forEach((i) => console.error(c('red', `  [CONFIG] ${i}`)));
} catch (e) {
  console.log(c('yellow', `  [WARN] ${e.message}`));
}

fs.writeFileSync('scan-unintended-files.report.json', JSON.stringify({ findings: allFindings }, null, 2));
